# simulate pathogen evolution and sampling
# simfixoutbreak function ------
# adapted from seedy

require(devtools)
#install_github("cran/seedy")
require(seedy)


simfixoutbreak2 <-
  function(ID, exp.times, inf.times, rec.times, inf.source, mut.rate, equi.pop, shape=flat,
           inoc.size, imp.var, samples.per.time, samp.schedule="random", 
           samp.freq, full, feedback, glen, 
           ref.strain=NULL, ...) {
    ...
    if (sum(exp.times>inf.times)>0) {
      stop("Exposed times must be earlier than infection times")
    }
    ...
    time <- min(exp.times) # in bacterial generations
    newinfect <- 0
    eff.cur.inf <- NULL
    init.inf <- sum(exp.times==min(exp.times))
    init.sus <- length(exp.times)-init.inf
    cur.inf <- ID[which(exp.times==min(exp.times))] # vector of infected person IDs
    n.sus <- length(exp.times)-init.inf
    tot.inf <- init.inf
    ...
    for (i in cur.inf) {
      ...
      if (w>1){
        totcurstrains <- c(totcurstrains,w)
      }
    }
    
    for (i in (1:length(exp.times))[-which(ID%in%cur.inf)]) {
      freq.log[[i]] <- 0
      strain.log[[i]] <- 0
    }
    ...
    while (time <= (max(rec.times)+1)) { # Cycle through generations until epidemic ceases
      ...
      if (time %in% exp.times) { # infection?
        n.sus <- n.sus-1
        infnow <- ID[which(exp.times==time)]
      ...
      if (length(cur.inf>0)) {
        ...
        for (i in cur.inf) {
          ...
          death.prob <- min(0.5 + 0.5*(pop.size-shape(time,span=rec.times[which(ID==i)]-exp.times[which(ID==i)]+1,equi.pop))/shape(time,span=rec.times[which(ID==i)]-exp.times[which(ID==i)]+1,equi.pop),1)
    ...
    if (full) {
      return(invisible(list(epidata=cbind(ID, exp.times, inf.times, rec.times, inf.source), 
